...
#
# ------------------------------------------------------------
#  本地 Watchdog vs 外部心跳 (Dead Man's Switch) 的区别:
#
#  本脚本 (bot_watchdog.sh):
#    - 运行在同一台服务器上
#    - 检测 Bot 进程崩溃/僵死, 自动拉起
#    - 局限: 如果整台服务器宕机、网络断开、或 watchdog 本身挂了,
#      则无法检测也无法告警
#
#  外部心跳 (HEARTBEAT_URL, 配置在 .env):
#    - Bot 每次扫描成功后 GET 外部 URL (如 healthchecks.io)
#    - 外部服务如果超时未收到 ping, 会通过邮件/Slack/PagerDuty 告警
#    - 能检测本地 watchdog 无法覆盖的场景:
#      服务器宕机、网络中断、磁盘满、OOM 等整体性故障
#
#  两者互补: 本地 watchdog 处理进程级故障, 外部心跳兜底整体宕机
# ============================================================
